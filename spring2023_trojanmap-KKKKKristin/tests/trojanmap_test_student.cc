...
TEST(TrojanMapStudentTest, Autocomplete1) {
  ...
  std::unordered_set<std::string> gt = {"Target","Tap Two Blue"};
  int success = 0;
  for(auto& n : names){
...
TEST(TrojanMapStudentTest, FindNearby1) {
  TrojanMap m;
  
  auto result = m.FindNearby("school", "National Schools", 5, 3);
  std::vector<std::string> ans{"358794547", "358793609","358791610"};
  EXPECT_EQ(result, ans);
}
TEST(TrojanMapStudentTest, FindNearby2) {
  TrojanMap m;
...
TEST(TrojanMapStudentTest, GetAllLocationsFromCategory1) {
  TrojanMap m;
...
TEST(TrojanMapStudentTest, GetAllLocationsFromCategory2) {
  TrojanMap m;
...
TEST(TrojanMapStudentTest, GetAllLocationsFromCategory3) {
  TrojanMap m;
